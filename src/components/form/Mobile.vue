<template>
  <modal :show="show" animate="right">
    <div ref="mobileLogin" class="hy-form">
      ...
      <div class="hy-form-body">
        ...
        >
          ...
          <btn
            ...
            @click="getCodeSM"
          />
          <div v-if="obj.msg !== ''" class="hy-form-item--error">
            {{ obj.msg }}
          </div>
        </div>
        <btn
...
export default class HyScenesMobileLogin extends Vue {
  ...
  private submit() {
    ...
    this.$emit('submit', { action: 'mobile', datas: { ...datas } });
  }
  ...
  private updated() {
    if (
      !this.show &&
      JSON.stringify(this.$data.form) !== JSON.stringify(defaultForm)
    ) {
      this.$data.form = deepCopy(defaultForm);
    }
  }
