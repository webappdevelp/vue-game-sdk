...
export default class Poly extends Mixins(pageCommonMix) {
      ...
      switch (action) {
        ...
        case 'pay':
          ...
          const result = await this.payRequest({
            ...
            ext: JSON.stringify({ uid, gameId, productName: datas.productName })
          });
  ...
  private sdkReport(params: {
    ...
    const { uid, gameId } = this.datas;
    window.CY_GAME_SDK.uploadGameRole({
      uid,
      gameId,
      ...
      gameRolePower: params.rolePower
    });
  }
  // sdk pay
  private sdkPay(params: any) {
    try {
      const { uid, signType, gameId } = this.datas;
      window.CY_GAME_SDK.pay({
        uid,
        gameId,
        signType,
        time: params.time,
  ...
  private async created() {
    try {
      await loadJS('//wap.hjygame.com/sdk/cy.sdk.js');
