...
export default class Poly extends Mixins(pageCommonMix) {
    ...
    if (action) {
      ...
      switch (action) {
        ...
        case 'pay':
          ...
          const { userId, userName, gameId } = this.datas;
          if (status === 0) {
            const { ext, order_id } = data;
            //返回pId,order_id,gameId,time, sign
            const payParams = JSON.parse(JSON.stringify(ext && ext[0]));
            const { gameDatas } = this.$data;
            payParams.source = gameDatas['source'];
            payParams.pId = payParams.pId || 5;
            payParams.userId = userId;
            payParams.userName = userName;
            payParams.gameId = payParams.gameId || 13;
            payParams.goodsId = datas.goodsId;
            payParams.goodsName = datas.productName;
            payParams.money = payParams.amount || datas.amount / 100;
            payParams.orderId = order_id;
            payParams.areaServer = datas.serverId;
            payParams.roleName = datas.roleName || datas.roleId;
            payParams.gameUrl = escape(window.location.href);
  ...
  private sdkPay(params: any) {
    let jump_url = 'http://www.foxwan.cn/Api/Leagues/Pay/trade?';
    for (let index in params) {
      jump_url = jump_url + index + '=' + params[index] + '&';
    }
    jump_url = jump_url.slice(0, jump_url.length - 1);
  ...
  private created() {
    ...
    this.$data.userInfo = {
      ...
      channelUserName: userName || userId || '',
      hyUid: userId || '',
      token: sign || ''
    };
    this.getInitData();
