...
export default class Poly extends Mixins(pageCommonMix) {
      ...
      switch (action) {
        ...
        case 'pay':
          ...
          if (status === 0) {
            ...
            payParams.uid = info.channelUserId;
            payParams.role_id = datas.roleId || '';
            payParams.role_name = '';
            payParams.gkey = this.datas.gkey;
            payParams.skey = this.datas.skey || 1;
  ...
  private sdkPay(params: any) {
    ...
    var jump_url = 'https://game.zixia.com/pay?';
  ...
  private created() {
    ...
    const { hy_gid, hy_channel_id, aid, start_origin, uid, sign } = this.datas;
