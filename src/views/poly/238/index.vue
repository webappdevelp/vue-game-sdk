...
export default class Poly extends Mixins(pageCommonMix) {
  ...
  // sdk登录
  private sdkLogin() {
    const message = JSON.stringify({ method: 'sdk_login' });
    window.webkit.messageHandlers.XWSDK.postMessage(message);
    prompt('XWSDK://sdk_login');
  }
  // sdk report
  private sdkReport(params: {
    ...
    window.webkit.messageHandlers.XWSDK.postMessage(message);
    prompt('XWSDK://sdk_serverandrole?' + message);
  }
  // 设置接口回调
  private listenEventCallBack() {
    window.sdk_login_result = async (res: string) => {
      const result = JSON.parse(res);
      const { userid, username, sign } = result;
      if (userid && sign) {
    ...
    window.sdk_pay_result = (status: number, message: string) => {
      if (status === 100) {
        this.postMessage({
          action: 'paySuccess'
        });
