...
export default class Poly extends Mixins(pageCommonMix) {
  ...
  // sdk share
  private share(params: { title: string; desc: string }) {
    const { gameDatas } = this.$data;
    const { uid, access_token, fullscreen } = this.datas;
    window.share_game.share_weibo({
      page_id: gameDatas['PAGE_ID'],
      app_key: gameDatas['APP_NO'],
      token: access_token,
      uid,
      content: params.desc,
      fullscreen,
  ...
  private sdkPay(params: {
    ...
    const { gameDatas } = this.$data;
    const { uid, access_token } = this.datas;
    window.top.location.href = `http://sng.sina.com.cn/payment/order/order_cashier?appkey=${
      gameDatas['APP_NO']
    }&access_token=${access_token}&uid=${uid}&amount=${params.amount ||
      0}&subject=${params.productName}&desc=${params.productDesc}&pt=${
      params.order_id
    }&timestamp=${params.timestamp}`;
  }
  private async created() {
    try {
      await loadJS('//mg.games.sina.com.cn/kjava/sng/js/share_game.js');
