...
export default class Poly extends Mixins(pageCommonMix) {
  ...
  private sdkPay(params: {
    ...
    const { username } = this.datas;
    const { gameDatas } = this.$data;
    window.top.location.href = `http://i.41188.com/pay/index?appid=${
      gameDatas['APP_ID']
    }&username=${username}&productname=${params.productName}&amount=${
      params.amount
    }&roleid=${params.roleId}&serverid=${params.serverId}&paytime=${
      params.paytime
    }&token=${params.sign}&attach=${params.order_id}`;
  }
  private created() {
