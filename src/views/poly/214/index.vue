...
export default class Poly extends Mixins(pageCommonMix) {
  ...
  private async dispatchMessage(e: MessageEvent) {
    ...
    cpDatas = cpDatas || {};
    const { action, datas, dataType, token, uid } = cpDatas;
    ...
    if (action) {
      ...
      switch (action) {
        ...
        case 'initSuccess':
          ...
          break;
        case 'login':
          const users = await this.loginRequest({
            ...this.datas
          });
          this.postMessage({
            action: 'loginSuccess',
        ...
        case 'pay':
          ...
          const result = await this.payRequest({
            ...
            ext: JSON.stringify({
              productId: datas.goodsId,
              productName: datas.productName,
              userType: 0,
              gameType: 4,
              passport: userInfo.channelUserName,
  ...
  private sdkReport(params: {
    ...
    window.parent.postMessage(
      JSON.stringify({
        data: {
          gameCode: gameDatas['CHANNEL_GAME_ID'],
          uid: userInfo.channelUserId,
          passport: userInfo.channelUserName,
          player_role_id: params.roleId,
          player_role_name: params.roleName,
          player_server_id: params.serverId,
          player_server_name: params.serverName,
          player_grade: params.level
        },
        dataType: 'getGameRole',
        frameDomain: `${window.location.protocol}//${window.location.host}`
      }),
      '*'
    );
  }
