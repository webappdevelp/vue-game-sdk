...
export default class Poly extends Mixins(pageCommonMix) {
  ...
  private async gameInit() {
    ...
    this.$data.sdkOptions = {
      ...sdkOptions,
      imei: `dangle_${new Date().getTime()}_${Math.ceil(Math.random() * 1000)}`
    };
  ...
  private sdkPay(params: any) {
    try {
      ...
      djGame.pay(
        {
          unionId: userInfo.channelUserId,
          amount: params.amount,
          cpOrder: params.order_id,
