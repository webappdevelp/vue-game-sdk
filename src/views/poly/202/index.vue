...
export default class Poly extends Mixins(pageCommonMix) {
  ...
  private async dispatchMessage(e: MessageEvent) {
    ...
    if (action) {
      const { avatar, cburl, reurl } = this.datas;
      ...
      switch (action) {
        ...
        case 'pay':
          const result = await this.payRequest({
            ...
            ext: JSON.stringify({ productName: datas.productDesc })
          });
  ...
  private sdkPay(params: any) {
    window.sdw.chooseSDWPay({
      ...
      paychannel: '',
      complete: async (res: { cpOrderId: string; timestamp: number }) => {
        this.checkOrder(res)
          .then(result => {
            const { status } = result;
            if (status === 0) {
  ...
  private async created() {
    try {
      await loadJS('//pay.17m3.com/gamepay/dhpay.min.js');
      await loadJS('//www.shandw.com/libs/js/sdwJs.min.js');
