...
export default class Poly extends Mixins(pageCommonMix) {
  ...
  private sdkReport(params: {
    ...
    try {
      window.CyUsdkHelper[params.cpAction]({
        server_id: params.serverId,
        server_name: params.serverName,
        role_id: params.roleId || '0',
        role_name: params.roleName || '0',
        role_level: params.level || '0',
        vip: params.vip || '0',
        role_ctime: params.roleCreateTime || 0,
        role_uptime: params.roleUpgradeTime || 0
      });
    } catch (err) {
  ...
  private async sdkLogin() {
    if (!window.CyUsdkHelper) {
      return this.updateToast('当前环境无法登录');
    }
    window.CyUsdkHelper.getUser(async (res: any) => {
      const { uid, session } = res;
  ...
  private sdkPay(params: any) {
    try {
      window.CyUsdkHelper.pay({
        cp_order_no: params.order_id,
        amount: params.amount,
        product_name: params.productName,
        product_num: 1,
        product_id: params.goodsId,
        product_price: 1,
        product_des: params.productDesc,
        game_coin: params.amount,
        cp_ext: params.order_id,
        rebirth_times: 0,
        server_id: params.serverId,
        server_name: params.serverName || params.serverId,
        role_id: params.roleId,
        role_name: params.roleName || params.roleId,
  ...
  private async created() {
    try {
      loadJS('//cdn3.91sd.com/static/usdk_helper.js');
