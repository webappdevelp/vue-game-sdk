...
export default class Poly extends Mixins(pageCommonMix) {
    ...
    if (action) {
      ...
      switch (action) {
        ...
        case 'pay':
          ...
          var wanDatas = this.datas;
          if (status === 0) {
            const { ext, order_id } = data;
            const payParams = JSON.parse(JSON.stringify(ext && ext[0]));
            payParams.userid = wanDatas.userid;
            payParams.pgid = wanDatas.pgid;
            payParams.serverid = wanDatas.serverid || datas.serverId;
            payParams.productname = datas.productName;
            payParams.productid = datas.goodsId;
            payParams.productnum = 1; //产品数量
            payParams.price = datas.amount;
            payParams.ext = order_id;
  ...
  private sdkPay(params: any) {
    ...
    var jump_url = 'http://liuliangjiayouzhan.cn/game/pay?';
