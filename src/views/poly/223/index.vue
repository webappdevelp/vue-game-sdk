...
export default class Poly extends Mixins(pageCommonMix) {
      ...
      switch (action) {
        ...
        case 'pay':
          ...
          if (status === 0) {
            ...
            payParams.app_order_no = order_id;
            payParams.app_subject = datas.productName;
            payParams.server_id = datas.serverId;
            payParams.server_name = datas.serverName || datas.serverId;
            payParams.role_id = datas.roleId;
            payParams.role_name = datas.roleName || datas.roleId;
            payParams.role_level = datas.level;
  ...
  private sdkPay(params: any) {
    try {
      window.RASTAR.pay({
        app_order_no: params.app_order_no,
        app_subject: params.app_subject,
  ...
  private sdkReport(datas: {
    ...
    roleUpgradeTime: string;
  }) {
    const { cpAction } = datas;
    let action = cpAction || 'enter';
    let role_info = {
      roleId: datas.roleId,
      roleName: datas.roleName,
      roleLevel: datas.level,
      serverId: datas.serverId,
      serverName: datas.serverName,
      balance: datas.balance || '0',
      vipLevel: datas.vip || '0',
      partyName: datas.partyName || '无',
      extra: '0',
      timeRoleCreate: datas.roleCreateTime || '-1',
      timeRoleUpgrade: datas.roleUpgradeTime || '-1'
    };
    window.RASTAR.upload({
      action,
      role_info
    });
  }
  private async created() {
    try {
      await loadJS('//tsource.rastargame.com/h5/game/rastar.js');
      ...
      this.$data.sdkOptions = {
        ...
        imei: `rastar_${new Date().getTime()}_${Math.ceil(
          Math.random() * 1000
        )}`,
        aid: aid || '0',
        start_origin
