...
export default class Poly extends Mixins(pageCommonMix) {
  ...
  // sdk share
  private share(params: any) {
    if (wan511SDK) {
      wan511SDK.share();
    } else {
      const { gameDatas } = this.$data;
      wan511SDK = new window.Wan511SDK(gameDatas['GAME_ID']);
      wan511SDK.share();
    }
  }
  // 设置分享回调
  private setShareCallBack() {
    window.onShareOk = () => {
      this.postMessage({
    ...
    window.onShareCallback = (data: any) => {
      this.postMessage({
        action: 'shareComplete',
        datas: data || 'success'
      });
  ...
  private sdkPay(params: {
    ...
    const { uid } = this.datas;
    const { gameDatas } = this.$data;
    window.top.location.href = `http://h5.511wan.com/pay.jsp?game_id=${
      gameDatas['GAME_ID']
    }&amount=${params.amount}&goods=${params.productName}&gold=1&time=${
      params.time
    }&sign=${params.sign}&uid=${uid}&gameOrderId=${params.order_id}`;
  }
  private async created() {
    try {
      let sdkUrl = 'https://weixinstatic.511wan.com/s/www/css/share.js';
      if (window.location.protocol === 'http:') {
        sdkUrl = 'http://src.cdn.n2k.cn/s/h5/js/share.js';
      }
      await loadJS(sdkUrl);
