...
import { apiPay, u9CreateOrder, wxPay } from '@/api/gamesPay';
...
export default class Index extends Mixins(PageCommonMix) {
	@Prop({
		type: Object,
		default: () => {
			return { aid: '0', gid: '0', channel: '154', startOrigin: 'grf' };
		}
	})
	private datas!: {
		aid: string;
	...
	private setShare() {
		...
		setShareInfo({
			...
			title: sdkGameDatas.title || '',
			desc: shareDesc[rang],
			link: window.location.href,
			imgUrl: `${
				window.location.origin
			}${require(`@/assets/games/${sdkOptions.app_id}/share.png`)}`,
			callback: () => {
				this.postMessage({
	...
	private centerAction({ action, params }: { action: string; params: any }) {
		...
		switch (action) {
			case 'wxTip':
				if (params) {
					if (!isWx) {
						return window.location.href = sdkGameDatas.kefu.link;
					}
					this.$data.wxTip = {
						show: true,
						msg: params
