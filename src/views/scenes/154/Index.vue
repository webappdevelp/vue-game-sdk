<template>
	<section class="scenes">
		<div style="display: none;">
			<img
				:src="sdkGameDatas.kefu && sdkGameDatas.kefu.wxqrcode"
				alt="微信二维码"
			/>
		</div>
		<hy-drag
...
export default class Index extends Mixins(PageCommonMix) {
	...
	// 重置充值中心面板数据
	/* @Watch('showPayPannel', { deep: true })
	resetPayPannelDatas() {
		const { showPayPannel } = this.$data;
		if (!showPayPannel) {
			this.$data.payPannelDatas = {};
		}
	} */
	// methods
	// message
	...
	private dispatchMessage(e: MessageEvent) {
		...
		if (action) {
			...
			switch (action) {
				...
				case 'pay':
					this.payCheck({
						cpOrderId: datas.cpOrderId || datas.productOrderId || '',
						amount: datas.amount || 0,
						body: datas.productDetail || datas.productDesc || '',
						subject: datas.productName || '',
						callback_url: datas.callBackUrl || datas.callbackUrl || '',
			...
			.then((res: { OrderId: string | undefined }) => {
				...
				if (!isWx) {
					...
					if (!!mapp) {
						...
						this.$data.payPannelExt = this.datas;
						this.$data.showPayPannel = true;
						return;
					}
					return apiPay({
						...sdkOptions,
				...
				if (isMiniProgam) {
					...
					return window.wx.miniProgram.navigateTo({
						url: `/pages/jumppay/index?app_id=${sdkOptions.app_id}&channel_id=${sdkOptions.channel_id}&app_order_id=${OrderId}&amount=${amount}&subject=${subject}&body=${body}&callback_url=${callback_url}&app_ext=${app_ext}&ext=''&guid=${guid}&aid=${sdkOptions.aid}&device=${sdkOptions.device}`
					});
				}
	...
	private created() {
		...
		// 更新游戏数据
		const { sdkGameDatas, adDatas } = this.$data;
		...
		this.$data.adDatas = {
			...adDatas,
			gameName: this.gameDatas.name,
			gameId: this.gameDatas.id,
			...this.datas
		};
		this.setShare();
	}
